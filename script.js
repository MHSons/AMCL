...
function loadLabSettings() {
    ...
    document.getElementById('labName').textContent = settings.name || config.appName;
    document.getElementById('labTagline').textContent = settings.tagline;
    document.getElementById('labLogo').src = settings.logo || config.defaultLogoBase64;
...
function checkReminders(role) {
    ...
    const patients = JSON.parse(localStorage.getItem('patients') || '[]');
    const now = new Date(); // 01:21 PM PKT
    const oneHourLater = new Date(now.getTime() + 60 * 60 * 1000); // 02:21 PM PKT
    console.log('Checking reminders at:', now.toLocaleString('en-US', { timeZone: config.timeZone }));
    ...
    patients.forEach(p => {
        ...
        if (appointmentTime > now && appointmentTime <= oneHourLater && p.status !== 'Completed') {
            ...
            const formattedTime = appointmentTime.toLocaleString('en-US', { timeZone: config.timeZone, dateStyle: 'medium', timeStyle: 'short' });
            reminder.innerHTML = `
